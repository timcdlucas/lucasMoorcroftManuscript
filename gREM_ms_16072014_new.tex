...
The gREM must be split into submodels rather than having one equation that describes $\bar{p}$ everywhere in parameter space. The reason for this is that there are situations where a small parameter change causes a different expression for $p$ to be needed rather than the size of $p$ changing smoothly as it would in the above expressions. We can see this in the equations by noting, for example, that $p=r\sin\frac{\alpha}{2}$ starts to get smaller as $\alpha$ increases beyond $\pi$. A wider animal signal width cannot give a decreased area that the animal can be detected in and so the equation must be changed. As a more visual example, if $\alpha$ was small, there would be an interval between Fig.~\ref{f:secondInt} and \ref{f:thirdInt} where the `blind spot' would prevent animals being detected at all giving a profile width of 0. This would require an extra integral in our equation for $\bar{p}$ as simply putting our small value of $\alpha$ into eqn \ref{e:SE2int} would not give us this integral of $p=0$.
