...
% @liz just checking you know there's two empty \ref{}s. One here and one somewhere else.
% @liz rowcliffe implies the derivation is theta < pi (but isn't totally explicit). In actual fact it is theta < pi/2, but marcus wasn't aware of that.
...
% @liz we should probably cite yapp1956theory here to make it clear that this isn't the first time this has been derived.
...
% @liz It's not for computational simplicity and it's needed for this model as well as later models. For the sensor to 'sweep' out an area, it needs to move. But as we're using a stationary sensor, it can only sweep out an area in terms of relative velocity.
