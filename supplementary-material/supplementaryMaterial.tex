...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{7cm}
        ...
        \includegraphics[width=1\textwidth]{imgs/AverageModelBias_speederror.pdf}
                ...
                \label{f:speed}
        \end{subfigure}%%
\caption{Model sensitivity (for all gREM submodels) to error in estimates of a) signal width ($\alpha$)  b) sensor width ($\theta$), c) sensor radius ($r$), d) animal movement speed ($v$). Estimates are -10\% (red), -1\% (orange), 0\% (yellow), +1\% (green) and +10\% (blue) of the true parameter value. The grey dashed line indicates zero error in density estimates. The error bars indicate standard error across all simulations.
%@tim double check this is standard error 
%@tim yellow and orange is similar
%@tim yellow is invisible
%@tim grey line to black and yellow to grey. 707371ff
}
