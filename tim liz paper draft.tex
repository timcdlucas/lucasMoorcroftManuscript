...
Four models, p141 (REM model), p343, p221, and p322, have been selected to demonstrate how the precision and accuracy varying with model inputs. Such as average distance travelled per animal during the survey time, density of the animal, sensor radius, tortuosity of movement, and proportion of time spent stationary. 

To be able to produce guidelines for best practise we need to identify the point where the standard deviation of the percentage error falls below an acceptable value. We have chosen this point to be the standard deviation less than five, in real terms this can be equated to saying that at this point if you were to same the same area multiple times 95\% of the density estimates would be within $\pm10\%$ of the true density. 
...
The appropriate length of the survey is closely linked with the average speed of the animal, \ref{e:D}, rather than looking at the two variables separately they can be studied together as the average distance moved by the animal within the duration of the survey. For all distances there is no significant deviation from the true value, and as the average distance moved during the survey increases the precision of the estimator increases, Figure~\ref{f:Distance}. The standard deviation for the error falls below five when the average distance moved per animal is greater than 40km, 55km, 25km and 25km for models p14, p343, p221, and p322 respectively. 


The density of the population also affects the precision of the estimates with the larger population densities being associated with greater precision. 
...
\begin{figure}[t]
        ...
        \begin{subfigure}[t]{0.5\textwidth}
                \centering
		\includegraphics[width=1\textwidth]{imgs/ResultsDistance.pdf}
                \caption{Distance}
                \label{f:Distance}
        \end{subfigure}
        ~ 
		...
		\includegraphics[width=1\textwidth]{imgs/ResultsDensitySubsample.pdf}
                ...
                \label{f:Density}
        \end{subfigure}
	\caption{Discrepency between known simulation density and estimated density with increasing animal movement (A) and animal density (B) with 95\% confidence intervals. As distance moved or animal density increases, precision of the estimate increases. However, the accuracy, or systematic bias, appears to be zero.}
	\label{}
\end{figure}
