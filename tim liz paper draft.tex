...
\begin{figure}[t]
        \centering
                ...
                \end{tikzpicture}
\caption{The location of the focal angle $x_1$ as used in models with $\theta>\pi$. This is the angle that is integrated over to find the average profile size. The sector  shaped detection region (with $\theta>\pi$) is shown in black. The widest part of this region (the profile) is shown with a thick red line and a blue rectangle. The direction of animal movement is downwards, as indicated by the grey arrow. %MR: I’m struggling to understand this figure – I’d understood it was a for a specific theta, for which the first picture makes sense, but the rest of them seem to have different thetas, which is doing my head in! Also I can’t grasp how the animal call zone interacts with this (I guess that’s what I’m failing to get my head round?). In general it’s a heroic effort to explain a really hard concept, but I’m not sure it’s quite there 
%@tim: This bit still needs to be been changed.
     }
\label{f:x1}
\end{figure}

We use $x_i$ to denote the focal angle which is the angle we integrate over. The subscript $i$ distinguishes different approach angles. For model SE2 we examine $x_1$ with  $x_1 = \pi/2$ being an approach angle directly towards the sensor (see Figure~\ref{f:x1}). 
