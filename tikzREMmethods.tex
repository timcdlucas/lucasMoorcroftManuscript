\usepackage{etoolbox}

\def\r{4}

\def\opac{0.2}
\def\lwr{-1.3*\r}
\def\upr{1.3*\r}
\def\gap{0.2}
\def\arrowSz{0.5}
\def\angRad{0.6}
\definecolor{arrowCol}{rgb}{0.5,0.5,0.5}
\definecolor{sensCol}{rgb}{0.6, 0.73, 0.89}
\definecolor{callCol}{rgb}{1,0,0}
\definecolor{profCol}{rgb}{0.8,0,0}
\definecolor{profText}{rgb}{0.55,0,0}
\definecolor{focus}{rgb}{0.5,0.8,0.5}

\tikzstyle{profile}=[ultra thick, profCol]



\newlength{\x}
\newlength{\y}

\newcommand{\call}[1]{ % callAngle
	\setlength{\y}{#1 pt}
	\ifboolexpr{
		test {\ifdimless{\y}{180 pt}}
	}{
		\pgfmathsetmacro{\width}{\r*sin(#1/2)};	
	}{
		\pgfmathsetmacro{\width}{\r};	
	} 
        \fill[opacity=\opac, callCol] (-{\width}, \lwr) rectangle ({\width}, \upr);       
}

\newcommand{\profileOne}[2]{ % sensor width, x1

	\setlength{\x}{#2 pt}
	\setlength{\y}{#1 pt}
	\ifboolexpr{%
		test {\ifdimless{0.5\y}{\x}} 
		and
		test{\ifdimless{\x}{360 pt -0.5\y}} 
	}{
		\pgfmathsetmacro{\leftProf}{max({\r*cos(#2 - #1/2)},{\r*cos(#2 + #1/2)})};
	}{
		\pgfmathsetmacro{\leftProf}{\r};
	}
	\draw[profile]  (0,0)  -- (180:\leftProf)
                        (0,0)  -- (0:\r);
}

\newcommand{\angleOne}[2]{
	\draw[profile] (0,0) ++  (-\angRad,0) arc (180:180 - #2 :\angRad);
        \node[below right, profText ] at (0,0) {$x_1$}
}



\newcommand{\sensorOne}[2]{ % sensorAngle, x1
	\setlength{\x}{#2 pt}
	\setlength{\y}{#1 pt}
	\ifboolexpr{%
		test {\ifdimless{0.5\y}{\x}} 
		and
		test{\ifdimless{\x}{360 pt -0.5\y}} 
	}{
		\pgfmathsetmacro{\leftProf}{max({\r*cos(#2 - #1/2)},{\r*cos(#2 + #1/2)})};
	}{
		\pgfmathsetmacro{\leftProf}{\r};
	}
        \fill[opacity=\opac, sensCol] (-\leftProf, \lwr) rectangle ({\r}, \upr);       
}

\newcommand{\segmentOne}[2]{ % sensor width, x1
	\draw[] (0,0) -- ++(180 + #1/2 - #2:\r)
     	        (0,0) -- ++(180 - #1/2 - #2:\r);
	\draw[] (0,0) -- (180 - #2 :\r);
	\draw[] (0,0) ++(180 + #1/2 - #2:\r) arc (180 + #1/2 - #2:180 - #1/2 - #2:\r);
}

\newcommand{\directionArrowOne}[3]{ % callAngle, sensorAngle, x1
	\setlength{\x}{#3 pt}
	\setlength{\y}{#2 pt}
	\ifboolexpr{%
		test {\ifdimless{0.5\y}{\x}} 
		and
		test{\ifdimless{\x}{360 pt - 0.5\y}} 
	}{
		\pgfmathsetmacro{\leftPoi}{max({\r*cos(#3 - #2/2)},{\r*cos(#3 + #2/2)}, {\r*sin(#1/2)})};
	}{
		\pgfmathsetmacro{\leftPoi}{\r};
	}
        \pgfmathsetmacro{\rightPoi}{max({\r*sin(#1/2)},{\r)})}
        \fill[ arrowCol ] (-\leftPoi, \lwr-\gap) -- (\rightPoi, \lwr-\gap) -- ({(-\leftPoi+\rightPoi)/2},\lwr-\gap - \arrowSz) -- cycle;

}


\newcommand{\profileTwo}[2]{ % sensor width, x2
        \pgfmathsetmacro{\pLen}{{2*\r*sin(#1/2)*sin(#2)}}
        \draw[profile] (0,0)  ++ (#2 - #1/2:\r) -- ++(180:\pLen) ;
}

\newcommand{\angleTwo}[2]{
	\draw[profile] (0,0) ++ (#2 + #1/2:\r) ++ (0,-\angRad) arc (-90:-(90 - #2):\angRad);
        \node[above right, profText] at (#2 + #1/2:\r) {$x_2$}
}


\newcommand{\sensorTwo}[2]{ % sensorAngle, x2
        \fill[opacity=\opac, sensCol] ({\r*cos(#1/2 + #2)}, \lwr) rectangle ({\r*cos(#2 - #1/2)}, \upr);       
}

\newcommand{\segmentTwo}[2]{ % sensor width, x2
	\draw[] (0,0) -- ++(#2 - #1/2:\r)
     	        (0,0) -- ++(#2 + #1/2:\r);
	\draw[] (0,0) ++(#2 - #1/2:\r) arc (#2 - #1/2:#2 + #1/2:\r);
	\draw[] (0,0) ++(#2 - #1/2:\r) -- (#2 + #1/2:\r);
}

\newcommand{\directionArrowTwo}[3]{ % callAngle, sensorAngle, x2
        \pgfmathsetmacro{\leftPoi}{min({\r*cos(#2/2 + #3)})}
        \pgfmathsetmacro{\rightPoi}{max({\r*cos(#3 - #2/2)})}
        \fill[ arrowCol ] (\leftPoi, \lwr-\gap) -- (\rightPoi, \lwr-\gap) -- ({(\leftPoi+\rightPoi)/2},\lwr-\gap - \arrowSz) -- cycle;

}

\newcommand{\profileThree}[2]{ % sensorAngle, x3
        \pgfmathsetmacro{\pLen}{{\r*sin(#2)}}
        \draw[profile] (0,0) ++ (90 - #2:\r) -- ++(180:\pLen) ;
}

\newcommand{\angleThree}[2]{
	\draw[profile] (0,0) ++  (0,\angRad) arc (90:90 - #2:\angRad);
        \node[below right, profText] at (0,0) {$x_3$}
}

\newcommand{\sensorThree}[2]{ % sensorAngle, x3
        \fill[opacity=\opac, sensCol] (0, \lwr) rectangle ({\r*sin(#2)}, \upr);       
}

\newcommand{\segmentThree}[2]{ % sensorAngle, x3
	\draw[] (0,0) -- ++(90 - #2 + #1:\r)
     	        (0,0) -- ++(90 - #2:\r);
	\draw[] (0,0) ++(90 - #2 + #1:\r) arc (90 - #2 + #1:90 - #2:\r);
	\draw[] (0,0) ++(90 - #2 + #1:\r) -- (90 - #2:\r);
}


\newcommand{\directionArrowThree}[3]{ % callAngle, sensorAngle, x3
        \pgfmathsetmacro{\leftPoi}{0}
        \pgfmathsetmacro{\rightPoi}{max({\r*sin(#2/2)},{\r*sin(#3)})}
        \fill[ arrowCol ] (\leftPoi, \lwr-\gap) -- (\rightPoi, \lwr-\gap) -- ({(\leftPoi+\rightPoi)/2},\lwr-\gap - \arrowSz) -- cycle;

}



\newcommand{\profileFour}[2]{ % sensor width, x4
	\setlength{\y}{#1 pt}
	\setlength{\x}{#2 pt}
	\ifboolexpr{%
		test {\ifdimgreater{\y}{90 pt}} 
		and
		test{\ifdimless{\x}{\y-90 pt}}  
	}{
		\pgfmathsetmacro{\leftPoi}{{\r*cos(#1-#2)}};
	}{
		\ifboolexpr{%
			test {\ifdimgreater{\y}{90 pt}} 
			and
			test{\ifdimgreater{\x}{90 pt}}
			}{
				\pgfmathsetmacro{\leftPoi}{{\r*cos(#2)}}; 
			}{ 		
				\pgfmathsetmacro{\leftPoi}{0};
			}
	}
        \draw[profile] (\leftPoi,0)  -- (0:\r) ;
}

\newcommand{\angleFour}[2]{
	\draw[profile] (0,0) ++ (\angRad,0) arc (0:-#2:\angRad);
        \node[above right, profText] at (0,0) {$x_4$}
}

\newcommand{\sensorFour}[2]{ % sensorAngle, x4
	\setlength{\y}{#1 pt}
	\setlength{\x}{#2 pt}
	\ifboolexpr{%
		test {\ifdimgreater{\y}{90 pt}} 
		and
		test{\ifdimless{\x}{\y-90 pt}}  
	}{
		\pgfmathsetmacro{\leftPoi}{{\r*cos(#1-#2)}};
	}{
		\ifboolexpr{%
			test {\ifdimgreater{\y}{90 pt}} 
			and
			test{\ifdimgreater{\x}{90 pt}}
			}{
				\pgfmathsetmacro{\leftPoi}{{\r*cos(#2)}}; 
			}{ 		
				\pgfmathsetmacro{\leftPoi}{0};
			}
	}
        \fill[opacity=\opac, sensCol] (\leftPoi, \lwr) rectangle ({\r}, \upr);       
}

\newcommand{\segmentFour}[2]{ % sensor width, x4
	\draw[] (0,0) -- ++(#1 - #2:\r)
     	        (0,0) -- ++(-#2:\r);
	\draw[] (0,0) ++(#1 - #2:\r) arc (#1 - #2:-#2:\r);
	\draw[] (0,0) ++(#1 - #2:\r) -- (-#2:\r);
}

\newcommand{\directionArrowFour}[3]{ % callAngle, sensorAngle, x4
\setlength{\y}{#2 pt}
	\setlength{\x}{#3 pt}
	\ifboolexpr{%
		test {\ifdimgreater{\y}{90 pt}} 
		and
		test{\ifdimless{\x}{\y-90 pt}}  
	}{
		\pgfmathsetmacro{\leftPoi}{{\r*cos(#2-#3)}};
	}{
		\ifboolexpr{%
			test {\ifdimgreater{\y}{90 pt}} 
			and
			test{\ifdimgreater{\x}{90 pt}}
			}{
				\pgfmathsetmacro{\leftPoi}{{\r*cos(#3)}}; 
			}{ 		
				\pgfmathsetmacro{\leftPoi}{0};
			}
	}
        \pgfmathsetmacro{\rightPoi}{max({\r*sin(#1/2)},{\r)})}
        \fill[ arrowCol ] (\leftPoi, \lwr-\gap) -- (\rightPoi, \lwr-\gap) -- ({(\leftPoi+\rightPoi)/2},\lwr-\gap - \arrowSz) -- cycle;

}





\newcommand{\fullOne}[3]{ % call, sensor, x4
        \call{#1};
        \sensorOne{#2}{#3};
        \directionArrowOne{#1}{#2}{#3};
        \segmentOne{#2}{#3};
        \profileOne{#2}{#3};
}



\newcommand{\fullTwo}[3]{ % call, sensor, x2
        \call{#1};
        \sensorTwo{#2}{#3};
        \directionArrowTwo{#1}{#2}{#3};
        \segmentTwo{#2}{#3};
        \profileTwo{#2}{#3};
}

\newcommand{\fullThree}[3]{ % call, sensor, x3
        \call{#1};
        \sensorThree{#2}{#3};
        \directionArrowThree{#1}{#2}{#3};
        \segmentThree{#2}{#3};
        \profileThree{#2}{#3};
}



\newcommand{\fullFour}[3]{ % call, sensor, x4
        \call{#1};
        \sensorFour{#2}{#3};
        \directionArrowFour{#1}{#2}{#3};
        \segmentFour{#2}{#3};
        \profileFour{#2}{#3};
}



\newcommand{\fullAngleOne}[3]{ % call, sensor, x4
        \fullOne{#1}{#2}{#3};
        \angleOne{#2}{#3};
}



\newcommand{\fullAngleTwo}[3]{ % call, sensor, x2
        \fullTwo{#1}{#2}{#3};
        \angleTwo{#2}{#3};
}

\newcommand{\fullAngleThree}[3]{ % call, sensor, x3
        \fullThree{#1}{#2}{#3};
        \angleThree{#2}{#3};
}



\newcommand{\fullAngleFour}[3]{ % call, sensor, x4
        \fullFour{#1}{#2}{#3};
        \angleFour{#2}{#3};
}


